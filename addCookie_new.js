 ...
 * ADD_COOKIE_USE_DAYS  是否开启了月付功能。（设置为false 或者 不设置时表示不开启月付功能。）
 * QUANTUM_INITIALIZE_USERS_DAYS 新用户给增加多少天。（需手动设置，天数 ，默认为 31 天）
 **/
...
let ADD_COOKIE_USE_DAYS = process.env.ADD_COOKIE_USE_DAYS || "flase";
...
您当前积分剩余${user.MaxEnvCount}`)
    ...
    if (ADD_COOKIE_USE_DAYS == "true") {
        if (ADD_COOKIE && !user) {
            sendNotify(`CK：${ADD_COOKIE}, 疑似从api接口匿名提交，结束！`, true);
            return;
        }
        current_time = moment().format('YYYY-MM-DD HH:mm:ss');
        if (!user.ExpiryDate) {
            console.log(`判定为新用户，做出标记！`);
            enable = true;
        } else if (user.ExpiryDate < current_time) {
            await sendNotify(`无法提交，你的账户已于${user.ExpiryDate}到期，请联系管理员续期。`);
            return;
        }
    }
    ...
    if (QuickLogin) {
        ...
        if (Phone && VerifyCode) {
            ...
            await sendNotify(`若你回复过验证码后无“提交成功”提醒
请稍后再试或使用其它登录方式`);
...
昵称：${$.nickName}`, true)
                            ...
                            if (enable) {
                                console.log(`判断该用户为新用户！为其增加${days}天`);
                                user.ExpiryDate = moment().add(days, "day").format('YYYY-MM-DD HH:mm:ss');
                                var updateInfo = await updateUserInfo(user);
                                if (updateInfo) {
                                    console.log(`新用户：${user.Id}增加 ${days} 天成功！到期时间：${user.ExpiryDate}`);
                                } else {
                                    console.log(`新用户：${user.Id}增加 ${days} 天失败了！！！！`);
                                    sendNotify(`新用户 ${user.Id} 增加失败了。`, true);
                                }
                            }
                            c.Id = null;
                            if (ADD_COOKIE_NOTIFY) {
                                if (enable) {
                                    await sendNotify(`新用户
Cookie新增通知
用户：${user_id}
昵称：${$.nickName}
到期时间：${user.ExpiryDate}`, true)
                                } else {
...
发生异常，系统错误：${data.Message}。`, true)
                    ...
                    await sendNotify("提交成功啦！\r京东昵称：" + $.nickName + beanNum + '\r京东数量：' + (jdCookies.length) + `\r到期时间：${user.ExpiryDate}`, false);
                    if (enable) {
                        await sendNotify(`检测你为新用户，为你赠送${days}天体验\n到期时间：${user.ExpiryDate}`, false);
                    }
                }
                else {
